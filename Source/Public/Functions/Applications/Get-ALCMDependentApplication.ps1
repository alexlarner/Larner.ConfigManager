function Get-ALCMDependentApplication {
    [CmdletBinding(
    ...
    process {
        ...
        foreach ($App in $Application) {
            ...
            $RequiredByApp = if ($IsADependency) {
                $AllApplicationsWithDependencies | Where-Object Dependencies -contains $App.ModelName
            }

            switch ($OutputType) {
                'Name' { $RequiredByApp.LocalizedDisplayName }
                'Object' { $RequiredByApp }
            }
        }
    }
}